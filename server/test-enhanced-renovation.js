/**
 * Test Enhanced AI Renovation Analysis with Market Data Integration
 * 
 * Tests:
 * 1. ATTOM market data retrieval
 * 2. Google contractor cost search
 * 3. GPT-5 image analysis
 * 4. Combined accurate metrics
 * 
 * Usage: node server/test-enhanced-renovation.js
 */

import fetch from 'node-fetch';
import fs from 'fs';
import path from 'path';

const SERVER_URL = process.env.VITE_PUSH_SERVER_URL || 'http://localhost:3001';

function normalizeCanonicalSuggestion(suggestion) {
  const canonicalResult = suggestion?.canonicalResult;
  if (!canonicalResult?.resultId) {
    return suggestion;
  }

  return {
    ...suggestion,
    id: canonicalResult.resultId,
    cost: canonicalResult.totalCost ?? suggestion.cost ?? 0,
    costRange: canonicalResult.costRange ?? suggestion.costRange,
    valueIncrease: canonicalResult.valueIncrease ?? suggestion.valueIncrease ?? 0,
    rentIncreaseDollar: canonicalResult.rentIncreaseDollar ?? suggestion.rentIncreaseDollar ?? 0,
    rentIncreasePercent: canonicalResult.rentIncreasePercent ?? suggestion.rentIncreasePercent ?? 0,
    currentRent: canonicalResult.currentRent ?? suggestion.currentRent ?? 0,
    maxPostRenovationRent: canonicalResult.maxPostRenovationRent ?? suggestion.maxPostRenovationRent ?? 0,
    roi: canonicalResult.roi ?? suggestion.roi ?? 0,
    paybackMonths: canonicalResult.paybackMonths ?? suggestion.paybackMonths ?? null,
    confidence: canonicalResult.confidence ?? suggestion.confidence,
    timeframe: canonicalResult.timeframe ?? suggestion.timeframe,
  };
}

// Sample small test image (1x1 red pixel PNG)
const SAMPLE_IMAGE = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8DwHwAFBQIAX8jx0gAAAABJRU5ErkJggg==';

async function testEnhancedRenovationAnalysis() {
  console.log('🏠 Testing Enhanced AI Renovation Analysis with Market Data\n');
  console.log('═══════════════════════════════════════════════════════════');
  console.log('This test validates:');
  console.log('  ✓ ATTOM API integration for local market data');
  console.log('  ✓ Google Search for contractor costs');
  console.log('  ✓ GPT-5 image analysis');
  console.log('  ✓ Combined accurate ROI/cost/rent lift metrics');
  console.log('═══════════════════════════════════════════════════════════\n');

  // Sample property data with a real address for ATTOM lookup
  const propertyData = {
    address: '1600 Pennsylvania Avenue NW, Washington, DC 20500', // White House for testing
    location: 'Washington, DC',
    monthlyRent: 2500,
    propertyValue: 450000,
    bedrooms: 3,
    bathrooms: 2,
    squareFeet: 1800,
    yearBuilt: 1995
  };

  // Try to load real test images
  let images = [];
  const testImageDir = path.join(process.cwd(), 'public', 'submissions');
  
  if (fs.existsSync(testImageDir)) {
    const files = fs.readdirSync(testImageDir)
      .filter(f => f.match(/\.(jpg|jpeg|png|gif)$/i))
      .slice(0, 2); // Take up to 2 images to save API costs
    
    if (files.length > 0) {
      console.log(`📁 Found ${files.length} test image(s) in public/submissions/\n`);
      images = files.map(file => {
        const filePath = path.join(testImageDir, file);
        const imageData = fs.readFileSync(filePath);
        const base64 = imageData.toString('base64');
        const ext = path.extname(file).slice(1).toLowerCase();
        const mimeType = ext === 'jpg' || ext === 'jpeg' ? 'image/jpeg' : `image/${ext}`;
        return `data:${mimeType};base64,${base64}`;
      });
    }
  }
  
  if (images.length === 0) {
    console.log('⚠️  No test images found, using sample image');
    console.log('   Add images to public/submissions/ for better testing\n');
    images = [SAMPLE_IMAGE];
  }

  const requestBody = {
    images,
    propertyData
  };

  console.log('📤 Sending analysis request...');
  console.log(`   Property: ${propertyData.address}`);
  console.log(`   Images: ${images.length}`);
  console.log(`   Expected integrations: ATTOM + Google + GPT-5\n`);

  try {
    const startTime = Date.now();
    
    const response = await fetch(`${SERVER_URL}/api/analyze-renovations`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify(requestBody)
    });

    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    
    console.log(`⏱️  Response time: ${duration}s`);
    console.log(`📊 Status: ${response.status} ${response.statusText}\n`);

    const result = await response.json();

    if (!response.ok || !result.ok) {
      console.error('❌ Error:', result.error || 'Unknown error');
      console.error('Full response:', JSON.stringify(result, null, 2));
      process.exit(1);
    }

    const suggestions = (result.suggestions || []).map(normalizeCanonicalSuggestion);

    console.log('✅ Analysis completed successfully!\n');
    
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('📊 DATA SOURCES USED:');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log(`   AI Model: ${result.model}`);
    console.log(`   Market Data: ${result.marketDataUsed ? '✓ ATTOM' : '❌ Not available'}`);
    console.log(`   Property Value: $${result.propertyValue?.toLocaleString() || 'N/A'}`);
    console.log(`   Estimated Rent: $${result.estimatedRent?.toLocaleString() || 'N/A'}/month`);
    console.log(`   Renovations Found: ${suggestions.length}\n`);

    if (suggestions.length > 0) {
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('🔧 RENOVATION SUGGESTIONS (Market-Validated):');
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

      suggestions.forEach((suggestion, idx) => {
        console.log(`${idx + 1}. ${suggestion.name}`);
        console.log(`   Priority: ${suggestion.priority.toUpperCase()} | Confidence: ${suggestion.confidence.toUpperCase()}`);
        console.log(`   Type: ${suggestion.type}`);
        console.log(`   Summary: ${suggestion.summary}`);
        if (suggestion.canonicalResult?.resultId) {
          console.log(`   Canonical Result: ${suggestion.canonicalResult.resultId}`);
        }
        console.log();
        console.log(`   💰 COST ANALYSIS:`);
        console.log(`      Estimated Cost: $${suggestion.cost.toLocaleString()}`);
        if (suggestion.costRange) {
          console.log(`      Range: $${suggestion.costRange.low.toLocaleString()} - $${suggestion.costRange.high.toLocaleString()}`);
        }
        console.log();
        console.log(`   📈 VALUE & ROI:`);
        console.log(`      Value Increase: $${suggestion.valueIncrease.toLocaleString()}`);
        console.log(`      ROI: ${suggestion.roi}%`);
        console.log(`      Payback Period: ${suggestion.paybackMonths ? suggestion.paybackMonths + ' months' : 'N/A'}`);
        console.log();
        console.log(`   🏠 RENT IMPACT:`);
        console.log(`      Current Rent: $${suggestion.currentRent.toLocaleString()}/month`);
        console.log(`      Rent Increase: +$${suggestion.rentIncreaseDollar.toLocaleString()}/month (+${suggestion.rentIncreasePercent}%)`);
        console.log(`      New Rent: $${suggestion.maxPostRenovationRent.toLocaleString()}/month`);
        console.log();
        console.log(`   📊 DATA SOURCES:`);
        if (suggestion.dataSource) {
          console.log(`      Contractor Cost Sources: ${suggestion.dataSource.contractorCosts} (Google Search)`);
          console.log(`      Market Data: ${suggestion.dataSource.marketData}`);
          console.log(`      AI Analysis: ${suggestion.dataSource.aiAnalysis}`);
        }
        console.log();
        console.log(`   ⏱️  Timeframe: ${suggestion.timeframe}`);
        console.log();
        if (suggestion.details) {
          console.log(`   📝 Details: ${suggestion.details.substring(0, 150)}${suggestion.details.length > 150 ? '...' : ''}`);
        }
        console.log();
      });

      // Summary stats
      const totalCost = suggestions.reduce((sum, s) => sum + (s.cost || 0), 0);
      const totalValueIncrease = suggestions.reduce((sum, s) => sum + (s.valueIncrease || 0), 0);
      const totalRentIncrease = suggestions.reduce((sum, s) => sum + (s.rentIncreaseDollar || 0), 0);
      const totalModeledReturn = suggestions.reduce((sum, s) => sum + (s.valueIncrease || 0) + ((s.rentIncreaseDollar || 0) * 12 * 5), 0);
      const overallROI = totalCost > 0 ? ((totalModeledReturn / totalCost) * 100).toFixed(1) : 0;
      const avgPayback = suggestions
        .filter(s => s.paybackMonths)
        .reduce((sum, s, _, arr) => sum + s.paybackMonths / arr.length, 0);

      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('📊 PORTFOLIO SUMMARY (All Renovations):');
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log(`   Total Investment: $${totalCost.toLocaleString()}`);
      console.log(`   Total Value Increase: $${totalValueIncrease.toLocaleString()}`);
      console.log(`   Overall Modeled ROI: ${overallROI}%`);
      console.log(`   Total Rent Increase: $${totalRentIncrease.toLocaleString()}/month`);
      console.log(`   Annual Rent Increase: $${(totalRentIncrease * 12).toLocaleString()}/year`);
      if (avgPayback > 0) {
        console.log(`   Average Payback: ${Math.round(avgPayback)} months (${(avgPayback / 12).toFixed(1)} years)`);
      }
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

      // Data quality assessment
      const highConfidence = suggestions.filter(s => s.confidence === 'high').length;
      const withContractorData = suggestions.filter(s => s.dataSource?.contractorCosts > 0).length;
      
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('📈 DATA QUALITY ASSESSMENT:');
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log(`   High Confidence: ${highConfidence}/${suggestions.length}`);
      console.log(`   With Contractor Data: ${withContractorData}/${suggestions.length}`);
      console.log(`   Market Data: ${result.marketDataUsed ? 'Yes (ATTOM)' : 'No (user-provided)'}`);
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    }

    console.log('✅ Test completed successfully!');
    console.log('\n💡 IMPROVEMENTS vs. previous version:');
    console.log('   ✓ Real contractor costs from Google Search');
    console.log('   ✓ Local market data from ATTOM API');
    console.log('   ✓ Market-based ROI calculations');
    console.log('   ✓ Data-driven rent increase estimates');
    console.log('   ✓ Confidence scoring based on data sources');
    console.log('   ✓ Cost ranges instead of single estimates');

  } catch (error) {
    console.error('❌ Test failed:', error.message);
    console.error('\nTroubleshooting:');
    console.error('  1. Is the backend server running? (npm run push-server)');
    console.error('  2. Check .env for required API keys:');
    console.error('     - OPENAI_API_KEY');
    console.error('     - ATTOM_API_KEY');
    console.error('     - GOOGLE_SEARCH_API_KEY');
    console.error('     - GOOGLE_CSE_CX');
    console.error('  3. Check server logs for errors');
    console.error(`  4. Try: curl ${SERVER_URL}/healthz`);
    process.exit(1);
  }
}

// Run the test
testEnhancedRenovationAnalysis();
