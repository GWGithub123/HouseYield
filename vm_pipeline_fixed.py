...
def find_largest_sparse_model(sparse_dir):
    """Find sparse model with most registered images (not points file size)"""
    ...
    for model_dir in model_dirs:
        # Prefer images.bin size as proxy for registered image count
        ...
        if images_file.exists():
            ...
            count = points_file.stat().st_size
        else:
            count = 0
        if count > best_count:
            best_count = count
            best_model = model_dir
        print(f"[Full Pipeline] Sparse model {model_dir.name}: images.bin={images_file.stat().st_size if images_file.exists() else 'N/A'}, points3D.bin={points_file.stat().st_size if points_file.exists() else 'N/A'}")
    
    return best_model or model_dirs[0]
...
def run_full_pipeline(images_dir, output_dir, quality='high'):
    ...
    # If Metric3D depth priors exist, they are already in the correct location
    # (dense/stereo/depth_maps/*.photometric.bin) and COLMAP will automatically
    # use them as initialization for patch_match_stereo — no special flags needed.
    if has_priors:
        print(f"[Full Pipeline] ✅ Metric3D depth priors found — COLMAP will use as initialization")
    else:
